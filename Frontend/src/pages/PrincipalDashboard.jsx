...
function PrincipalDashboard() {
  ...
  const fetchGrievances = async () => {
    try {
      const res = await api.get('/principal/grievances');
      setGrievances(res.data);
    } catch (err) {
      console.error(err);
      toast.error('Failed to load Principal escalated grievances');
    }
  };
  ...
  const handleAction = async (id, action) => {
    setLoading(true);
    try {
      let endpoint = `/principal/grievances/${id}/resolve`;
      if (action === 'close') endpoint = `/principal/grievances/${id}/close`;
    ...
    <div>
      <header className="header">
        <div>
          ...
          <p style={{ color: 'hsl(var(--text-secondary))', marginTop: '4px' }}>College-wide escalated complaints management</p>
        </div>
      ...
      <div className="stats-bar">
        <div className="stat-card">
          ...
          <p>Total College Escalations</p>
        </div>
        <div className="stat-card">
          <h3>{grievances.filter(g => g.status === 'ESCALATED_TO_PRINCIPAL').length}</h3>
          <p>Pending Review</p>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          <h3>{grievances.filter(g => g.status === 'CLOSED').length}</h3>
          <p>Closed</p>
        </div>
      </div>
      ...
      <div className="card">
        <h2 style={{ marginBottom: '20px', fontFamily: 'var(--font-display)' }}>Principal Escalations Queue</h2>
        {filteredGrievances.length === 0 ? (
          <p style={{ color: 'hsl(var(--text-muted))', textAlign: 'center', padding: '40px 0' }}>
            No escalated grievances match your filter criteria.
          </p>
        ) : (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', minWidth: '150px' }}>
                  {g.status === 'ESCALATED_TO_PRINCIPAL' && (
                    <>
                      ...
                      <button 
                        onClick={() => handleAction(g.id, 'close')}
                        className="btn btn-outline"
                        style={{ fontSize: '0.85rem', padding: '8px 12px', color: 'white', borderColor: 'hsl(var(--border))' }}
                        disabled={loading}
                      >
                        🔒 Close Ticket
                      </button>
