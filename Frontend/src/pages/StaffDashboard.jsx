...
function StaffDashboard() {
  ...
  const [filterAnon, setFilterAnon] = useState('ALL'); // 'ALL', 'ANON', 'NON-ANON'
  ...
  const fetchGrievances = async () => {
    try {
      const res = await api.get('/staff/grievances');
  ...
  const handleStatusChange = async (id, action) => {
    setLoading(true);
    try {
      let endpoint = `/staff/grievances/${id}/in-progress`;
      if (action === 'resolve') endpoint = `/staff/grievances/${id}/resolve`;
      if (action === 'escalate') endpoint = `/staff/grievances/${id}/escalate`;
      ...
      console.error(err);
      toast.error(err.response?.data?.message || 'Failed to update grievance status');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      <header className="header">
        <div>
          ...
          <p style={{ color: 'hsl(var(--text-secondary))', marginTop: '4px' }}>Review and resolve assigned student grievances</p>
        </div>
      ...
      <div className="card">
        <h2 style={{ marginBottom: '20px', fontFamily: 'var(--font-display)' }}>Assigned Grievances</h2>
        {filteredGrievances.length === 0 ? (
          <p style={{ color: 'hsl(var(--text-muted))', textAlign: 'center', padding: '40px 0' }}>
            No grievances match your filter parameters.
          </p>
        ) : (
            ...
            {filteredGrievances.map((g) => (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', minWidth: '150px' }}>
                  {g.status === 'OPEN' && (
                    <button 
                      onClick={() => handleStatusChange(g.id, 'in-progress')}
                  ...
                  {g.status === 'IN_PROGRESS' && (
                    <>
                      <button 
                        onClick={() => handleStatusChange(g.id, 'resolve')}
                        className="btn btn-primary"
                      ...
                      <button 
                        onClick={() => handleStatusChange(g.id, 'escalate')}
